      ...
      for (int j = 0; j < ((p_treenode)filetree.get(i)).size(); j++) {
        ...
        if (tn.checked) {
          ...
          for (int k = 0; k < p.bayesintrange.length; k++) {
            chrs[p.chr_chrs[k]-1].add(p.chr_peaks[k], p.bayesintrange[k], tn.drawcolor, (float)x+(cw*((p.chr_chrs[k]-1)%chr_columns))+16/*+(cw/2.0)*/,
              (multiplier*(p.bayesintrange[k].lower))+(float)y+(ch*floor((p.chr_chrs[k]-1)/(float)chr_columns))+20, 
              multiplier*(p.bayesintrange[k].upper - p.bayesintrange[k].lower),
              (multiplier*((p.chr_peaks[k]-1)))+(float)y+(ch*floor((p.chr_chrs[k]-1)/(float)chr_columns))+20);
          }
        }
      }
