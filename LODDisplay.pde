class LODDisplay implements p_component {
  ...
  void update() {
    ...
    strokeWeight(1);
    //double current_x = 0;
    int xNum = chr_lengths.length;
    for (int i = 0; i < chr_names.length; i++)
      if (chr_names[i].equalsIgnoreCase("x")) xNum = i;
    if (current_chr == -1)
      for (int i = 0; i < chr_lengths.length; i++) {
        ...
        float pos = map(chr_offsets[i]+(chr_lengths[i]/2.0), 0.0, chr_total, 0.0, (float)w);
        //if (i == chr_lengths.length-1) text("X", (float)x+pos-(textWidth("X")/2.0), height-64);
        //else 
        text(chr_names[i], (float)x+pos-(textWidth(chr_names[i])/2.0), (float)(y+h)-14);
      ...
      for (int i = 0; i < phenos.size(); i++) {
        for (int j = 0; j < ((p_treenode)filetree.get(i)).size(); j++) {
          ...
          if (tn.checked) {
            ...
            textFont(smallFont);
            if (current_chr == -1) {
              float endX = (p.thresholds.length > 1 && p.thresholds[0][0] == p.thresholds[1][0] && p.thresholds[0][1] == p.thresholds[1][1]) ? (float)(x+w) : map(chr_offsets[xNum], 0.0, chr_total, (float)x, (float)(x+w));
            ...
            stroke(tn.drawcolor);
            strokeWeight(1);
            float lastx = map(p.position[0]+chr_offsets[p.chromosome[0]-1], 0.0, chr_total, 0.0, w), lasty = map(p.lodscores[0], 0.0, m, 0.0, h-50);
            ...
            for (int k = 1; k < p.position.length; k++) {
              try {
                if (current_chr != -1) {
                  ...
                  lastx = map(p.position[k]+chr_offsets[p.chromosome[k]-1], 0.0, chr_total, 0.0, w);
                  lasty = map(p.lodscores[k], 0.0, m, 0.0, h-50);
                } else line((float)x+lastx, (float)(y+h)-lasty-50.0, (float)x+(lastx = map(p.position[k]+chr_offsets[p.chromosome[k]-1], 0.0, chr_total, 0.0, w)), (float)(y+h)-(lasty = map(p.lodscores[k], 0.0, m, 0.0, h-50))-50.0);
  ...
  void mouseAction() {
    ...
    if (mouseX > x && mouseY > y && mouseX < x+w && mouseY < y+h-50 && active && mousePressed && mouseButton == LEFT && !dragging && chr_ready && current_chr == -1) {
        for (int i = 0; i < chr_offsets.length; i++) {
          if (mouseX > map(chr_offsets[i], 0.0, chr_total, 0.0, width-50-(float)x)+x) {
            if (i == chr_offsets.length - 1)
              current_chr = i;
            else if (mouseX < map(chr_offsets[i+1], 0.0, chr_total, 0.0, width-50-(float)x)+x)
              current_chr = i;
          }
       }
    }
  ...
  void nextChr() {
    if (/*folder*/TEST.currentpage != 0) return;
